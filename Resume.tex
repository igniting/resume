...
\textbf{AdWyze} \hfill \textit{\textbf{Senior Software Engineer}, July 2016 - Present} \\
\vspace{-0.6cm}
\begin{itemize}
\setlength{\itemsep}{0.1mm}
	\item Integrated multiple advertising and analytics channels on the AdWyze platform, written in \textit{Ruby on Rails}. Created a generic framework for addition of new advertising and analytics channels
	\item Worked on handling several performance issues on backend consisting of PostgreSQL DB, MongoDB and the
background jobs framework. This also involved setting up monitoring for key backend metrics using Grafana, Graphite and collectd
\end{itemize}

\textbf{LinkedIn} \hfill \textit{\textbf{Software Engineer}, July 2015 - June 2016} \\
\vspace{-0.6cm}
\begin{itemize}
\setlength{\itemsep}{0.1mm}
	\item Worked in the communication team which handles LinkedIn's outbound emails, SMS and push notifications. Integrated several external SMS providers using SMPP protocol. Wrote generic code for adding new SMS providers
	\item Worked on several strategies to bring down failure rates of outbound SMS, including building a retry mechanism using Kafka
	\item Worked with pig and hive to analyze different events generated by the communication team
\end{itemize}
...
\textbf{Library for handling large binary objects in Haskell} \textit{(M.Tech Thesis)} \hfill \textit{July 2014 - May 2015}
\vspace{-0.2cm}
\begin{itemize}
\setlength{\itemsep}{0.1mm}
	\item Built a library for handling large binary objects (blob) in Haskell
	\item Provided lock free concurrent access to blobs
\end{itemize}
\textbf{Distributed web application for IIT Kanpur} \hfill \textit{Dec 2012 - May 2015}
\vspace{-0.2cm}
\begin{itemize}
\setlength{\itemsep}{0.1mm}
